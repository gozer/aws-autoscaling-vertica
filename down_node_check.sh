...
function get_instance_by_ip() {
   ip=$1
   instanceId=$(aws --output=text ec2 describe-instances --filters "Name=network-interface.addresses.private-ip-address,Values=$ip" --query "Reservations[*].Instances[*].InstanceId")
   echo "$instanceId"
}
